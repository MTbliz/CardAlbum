...
class CardsController:
    ...
    def create_card(self):
        ...
        if search_form.validate_on_submit():
            session['searched_title'] = request.form.get('title', "")
            session['searched_set'] = request.form.get('set', "All")
            return redirect(url_for('card.create_card'))

        searched_title = session.get('searched_title', "")
        searched_set = session.get('searched_set', "All")

        search_form.title.data = searched_title
        search_form.set.data = searched_set
        card_image = 'img/img.png'

        card_details = self.card_service.get_card_details_from_url(base_link, self.card_service.get_set_value_by_name(searched_set), searched_title)
